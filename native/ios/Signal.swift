...
//  }
  ...
  func decryptMessage(
    ...
    if type == 3 {
      decryptedMessage = try! ownSessionCipher.decrypt(preKeySignalMessage: Data(base64Encoded: payload)!)
    } else {
      decryptedMessage = try! ownSessionCipher.decrypt(signalMessage: Data(base64Encoded: payload)!)
    }
  ...
  func encryptPayload(
    ...
    let encryptedMessage = try! cipher.encrypt(payload.data(using: .utf8)!)
